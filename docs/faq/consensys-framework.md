...
For more details, please refer to the [Validator section](/docs/maintain/validate/getting-started.md).
...
[End-to-end flow to a Polygon validator](/docs/maintain/polygon-basics/who-is-validator.md#end-to-end-flow-for-a-polygon-validator) and [Responsibilities of a validator](/docs/maintain/validate/validator-responsibilities.md).
...
Please refer to [What is the incentive](/docs/maintain/validator/rewards.md#what-is-the-incentive) for more details.
...
For more details, refer to [Steps to use the POS bridge](/docs/develop/ethereum-polygon/pos/getting-started.md#steps-to-use-the-pos-bridge).

### - How soon are those funds available on the Polygon PoS?

Around ~22-30 minutes. This is done via a message passing mechanism that is termed as `state sync`. More details can be found [here](/docs/pos/state-sync/state-sync-mechamism.md).
...
You can read about them [here](/docs/develop/ethereum-polygon/mintable-assets.md).

### - How would a user dispute an invalid Polygon PoS transaction? Prove a valid Polygon PoS transaction?

There is currently no way on-chain to dispute an invalid Polygon PoS transaction. However, validators of the Polygon PoS chain submit periodic checkpoints to Ethereum - you can see more details [here](/docs/pos/heimdall/modules/checkpoint.md).
...
Approximately ~1-3 hours depending on the frequency of the [checkpoints](/docs/pos/heimdall/modules/checkpoint.md). The frequency is majorly a function of the cost that the validators are willing to spend on ETH gas fees to submit checkpoints.
...
There are already a few players such as [Connext](https://connext.network/) and [Biconomy](https://biconomy.io/) that are or will be providing this service. There are a various number of other players who are also going to go live pretty soon.
